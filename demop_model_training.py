...
# ------------------ 5. 构建完整 Dataset，并划分 Train/Val/Test ------------------
dataset = TextDataset(
...
# 建立原始数据（data）的 index 到 dataset 内部位置的映射
index_to_position = {idx: pos for pos, idx in enumerate(data.index)}

# 读取已有的 train_data 与 test_data CSV 文件（注意：CSV 文件中使用了原始数据的 index）
...
# 根据映射将 CSV 中的 index 转换为 dataset 中的下标
...
print(f"train_dataset样本数: {len(train_dataset)}")
print(f"val_dataset样本数: {len(val_dataset)}")
print(f"test_dataset样本数: {len(test_dataset)}")

# ------------------ 6. 构造 DataCollator (自动padding) ------------------
data_collator = DataCollatorWithPadding(tokenizer=tokenizer, return_tensors="pt")

# ------------------ 7. 自定义 Attention 模块 ------------------
class Attention(nn.Module):
    """
    简单的Attention模块，将 [batch_size, seq_len, hidden_dim] 的输出
    经过Attention池化得到 [batch_size, hidden_dim] 的句向量表示。
    """
...
# ------------------ 8a. 定义 ResNet-style Residual Block for FC layers ------------------
...
# ------------------ 8b. 定义 ResNet-style 分类头 ------------------
class ResNetClassifier(nn.Module):
    def __init__(self, dropout_rate=0.1):
        super(ResNetClassifier, self).__init__()
        # 两个残差块：分别将尺寸从1024->512和512->256
...
# ------------------ 8. 定义 ExpertNetwork（简化版） ------------------
class ExpertNetwork(nn.Module):
    """
    专家网络：采用 ResNet 风格的结构从 1024 -> 512 -> 256 -> 2.
    """
...
# ------------------ 9. 定义 Mixture of Experts（MoE）模块 ------------------
class MixtureOfExperts(nn.Module):
    ...
    def forward(self, x):
        ...
        expert_outputs = [expert(x) for expert in self.experts]   # 每个元素形状为 [batch_size, num_classes]
        ...
        output = torch.sum(gating_weights * expert_outputs, dim=1)  # 加权求和，得到 [batch_size, num_classes]
        return output

# ------------------ 10. 定义整体模型 CustomDebertaV3MoEForSequenceClassification ------------------
class CustomDebertaV3MoEForSequenceClassification(nn.Module):
    def __init__(self, num_experts=4):
        ...
        # 直接从 4096 降到 1024，不使用残差 shortcut
        self.mlp_reduce = nn.Sequential(
            nn.Linear(4096, 1024),
            nn.GELU()
        )
        
        hidden_size = 1024  # 降维后的 hidden size
...
# ------------------ 22. 针对每个 CANCER_TYPE_DETAILED 评估 ------------------
...
# ------------------ 23. 预测测试集并计算整体 Accuracy + 类别1 F1 + AUC ------------------
...
# ------------------ 25. 按 CANCER_TYPE 细分计算 Accuracy / 类别1 F1 / AUC ------------------
...
# ------------------ 26. 按 CANCER_TYPE_DETAILED 细分计算 Accuracy / 类别1 F1 / AUC ------------------
